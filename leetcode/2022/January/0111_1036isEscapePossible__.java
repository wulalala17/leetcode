...
class Solution {//评论区带限制的BFS 好难
    ...
    boolean bfs(int[][] blocked, int[] source, int[] target) {
        ...
        for (int[] block : blocked) visited.add(1000000 * block[0] + block[1]);
        visited.add(1000000 * source[0] + source[1]);
        Queue<int[]> queue = new ArrayDeque<>();
        queue.offer(source);
        while (!queue.isEmpty()) {
            int size = queue.size();
            if (size >= blocked.length) return true;// 包不住了
            while (--size >= 0) {
                int[] p = queue.poll();
                for (int[] d : DIRECTIONS) {
                    ...
                    if (visited.add(1000000 * x + y)) queue.offer(new int[]{x, y});
                }
            }
        }
        return false;
